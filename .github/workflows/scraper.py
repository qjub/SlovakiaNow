...
def run_scraper():
    ...
    if r:
        try:
            ...
            output["ekonomika"]["hicp_eurostat"] = points
    ...
    # ── Eurostat — HDP rast Slovensko YoY% ───────────────────────────────────
    # unit=PCH_SM_PER = medziročný rast (YoY %) — správna metrika pre dashboard
    # Poznámka: PCH_PRE_PER = QoQ%, CLV_PCH_SM_PER = reálny YoY%
    log.info("📊 Eurostat — HDP rast SK (YoY%)...")
    hdp_url = (
        "https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/"
        "namq_10_gdp/Q.CLV_PCH_SM_PER.SCA.B1GQ.SK"
        "?format=JSON&lastTimePeriod=20"
    )
    r = safe_get(hdp_url)
    if r:
        try:
            d = r.json()
            # Eurostat JSON: dimension.time.category.index = {perioda: poradie}
            time_dim = d.get("dimension", {}).get("time", {})
            time_index = time_dim.get("category", {}).get("index", {})
            # index je dict {label: position}, treba ho otočiť
            idx_to_label = {v: k for k, v in time_index.items()}
            values_raw = d.get("value", {})
            if isinstance(values_raw, list):
                values_raw = {str(i): v for i, v in enumerate(values_raw)}
            points = []
            for str_i, val in sorted(values_raw.items(), key=lambda x: int(x[0])):
                if val is not None:
                    label = idx_to_label.get(int(str_i), str_i)
                    points.append({"perioda": label, "hodnota": round(float(val), 2)})
            output["ekonomika"]["hdp_stvrtrocne"] = points[-20:]
            output["meta"]["zdroje"].append({"nazov": "Eurostat — HDP rast SK (YoY%)", "url": "https://ec.europa.eu/eurostat", "format": "SDMX JSON"})
            log.info(f"  ✅ HDP Eurostat: {len(points)} bodov, posledný: {points[-1] if points else 'N/A'}")
        except Exception as e:
    ...
    # nrg_pc_204: ceny elektriny pre domácnosti, SK, EUR/kWh vrátane daní
    # Správny filter: consom=KWH2500-4999 (nie siec)
    log.info("⚡ Eurostat — Ceny elektriny...")
    el_url = (
        "https://ec.europa.eu/eurostat/api/dissemination/sdmx/2.1/data/"
        "nrg_pc_204/S.I_TAX.KWH.EUR.SK"
    ...
    if r:
        try:
            ...
            for str_i, val in sorted(values_raw.items(), key=lambda x: int(x[0])):
                if val is not None:
                    label = idx_to_label.get(int(str_i), str_i)
                    # Eurostat hodnoty sú v EUR/kWh, prevedieme na ct/kWh
                    points.append({"perioda": label, "hodnota": round(float(val) * 100, 2)})
            output["energie"]["elektrina_centkwh"] = points
            log.info(f"  ✅ Elektrina: {len(points)} bodov, posledný: {points[-1] if points else 'N/A'}")
        except Exception as e:
            output["errors"].append(f"Eurostat energia: {e}")
            log.error(f"  ❌ Eurostat energia: {e}")
